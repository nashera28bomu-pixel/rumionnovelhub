...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({ success: true, message: 'Novel created successfully', data });
    } catch (error) {
...
router.post('/chapter', authenticate, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Chapter added successfully', data });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
⚠️  MUST stay above /:id so Express doesn't match "chapter" as an id
*/
...
/*
|--------------------------------------------------------------------------
GET SINGLE NOVEL + ITS CHAPTERS (metadata only, no content)
*/
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (novelError) {
      return res.status(404).json({ success: false, message: novelError.message });
    }
    ...
    if (chapterError) {
      return res.status(500).json({ success: false, message: chapterError.message });
    }

    res.json({ success: true, data: { novel, chapters } });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Novel updated', data });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
});
